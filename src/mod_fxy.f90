...
contains
   ...
   subroutine fxy_rwg(wfld,dfld,dt,th0,dth,joff,nlon,nlat,istep,fg)
      ...
      real(kind=REAL_BYTE), pointer, dimension(:,:) :: fx_old, fx_new, fy_old, fy_new
...
#else
!$omp target teams distribute parallel do collapse(2) private(i,fybar,cf,bcf,fric,ddx_tmp,d,lim)
#endif
         do j = jst, jnd
            do i = ist, ind
               fybar = quart*(fy_old(i,j) + fy_old(i+1,j) + fy_old(i,j-1) + fy_old(i+1,j-1))
...
#else
                     ! cobined semi-implicit added on 14/04/2019, Baba
!                 fric = dt*bcf*half*sqrt(fx_old(i,j)*fx_old(i,j)+fybar*fybar)/ddx_tmp/ddx_tmp
...
#else
!$omp target teams distribute parallel do collapse(2) private(i,fxbar,cf,bcf,fric,ddy_tmp,d,lim)
#endif
         do j = jst, jnd
            do i = ist, ind
               fxbar = quart*(fx_old(i,j) + fx_old(i-1,j) + fx_old(i-1,j+1) + fx_old(i,j+1))
...
#else
                     ! conbined semi-implicit added on 14/04/2019, Baba
                     !fric = dt*bcf*half*sqrt(fy_old(i,j)*fy_old(i,j)+fxbar*fxbar)/ddy_tmp/ddy_tmp
                     ! simple emi-implicit added on 16/05/2023, Minami
