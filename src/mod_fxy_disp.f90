...
#ifdef NORMALMODE
#ifdef USE_GPU
         write(0,'(a)') 'ERROR! NORMALMODE is NOT supported on GPU!'; stop
#endif
         nm_ind => wfld%nm_ind
         nm_P   => wfld%nm_P
         nm_P0  => wfld%nm_P0
...
#else
!$omp target teams distribute private(theta,i,c0,c1)
#endif
            do j = jst, jnd
#ifdef USE_GPU
               do i = ist, ind
...
#else
#ifdef USE_GPU
         write(0,'(a)') 'ERROR! BANKFILE is NOT supported on GPU!'; stop
#endif
...
!$omp private(theta,sint,invst,gdtdss,crls,i,fybar,dvdx,dvdy,advc,tmp0,tmp1)
#endif
#else
#ifndef USE_GPU
!$omp do &
...
#else
!$omp target teams distribute parallel do collapse(2) private(theta,sint,invst,gdtdss,i,dh)
#endif
         do j = jst, jnd
#ifdef USE_GPU
            do i = ist, ind
...
#else
!$omp target teams distribute parallel do collapse(2) &
!$omp private(theta,sint,invst,crls,i,fxbar,dvdx,dvdy,advc,tmp0,tmp1)
#endif
#else
#ifndef USE_GPU
!$omp do &
...
#else
!$omp target teams distribute parallel do collapse(2) &
!$omp private(theta,sint,invst,crls,i,fxbar,dvdx,dvdy,advc,tmp0,tmp1,tmp2,tmp3)
#endif
#endif
         do j = jst, jnd
#ifdef USE_GPU
...
!$omp do private(i, dh)
#else
!$omp target teams distribute parallel do collapse(2) private(i,dh)
#endif
         do j = jst, jnd
            do i = ist, ind
...
#else
!$omp target teams distribute parallel do collapse(2) private(theta,sint,i,fybar,cf,bcf,fric,ddx_tmp,d,lim)
#endif
         do j = jst, jnd
#ifdef USE_GPU
            do i = ist, ind
...
#else
!$omp target teams distribute parallel do collapse(2) private(theta,sint,i)
#endif
            do j = jst, jnd
#ifdef USE_GPU
               do i = ist, ind
...
#else
!$omp target teams distribute parallel do collapse(2) private(theta,sint,i,fxbar,cf,bcf,fric,ddy_tmp,d,lim)
#endif
         do j = jst, jnd
#ifdef USE_GPU
            do i = ist, ind
...
!$omp do private(dh)
#else
!$omp target teams distribute parallel do private(dh)
#endif
            do i = ist, ind
               if(ifz(i,j) > 0 .and. ifz(i,j+1) < 0) then ! wet north, dry south
                  dh = dz(i,j+1) + hz_old(i,j)
...
#else
!$omp target teams distribute parallel do private(fxbar,fybar,cf,cf2,bcf,bcf2,fric,fric2,ddx_tmp,ddy_tmp)
#endif
#endif
#ifndef MPI
            do i = 1, nlon-1
...
#else
!$omp target teams distribute parallel do private(theta,sint,gdtdss)
#endif
#else
#ifndef USE_GPU
!$omp do private(theta, sint, gdtdss, crls, fxbar, fybar, cf, cf2, bcf, bcf2, fric, fric2, ddx_tmp, ddy_tmp)
#else
!$omp target teams distribute parallel do private(theta,sint,gdtdss,crls,fxbar,fybar,cf,cf2,bcf,bcf2,fric,fric2,ddx_tmp,ddy_tmp)
#endif
#endif
#ifndef UPWIND3
            do j = 2, nlat-2
